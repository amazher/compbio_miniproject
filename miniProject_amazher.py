''' Python wrapper for CompBio 383 mini-project. This script will automate the retrieval and analysis of HCMV transcriptomes
    using programs such as kallisto, sleuth, Bowtie2, and SPAdes. '''
...
if prompt == 'test':
    # go back up one folder (so script can see test_data folder)
    os.chdir('..') 
    # use shutil to copy and rename the test files and use those in the code below
...
# if the user wants to run the full dataset, download full data from ncbi and continue with script
    ...
    # retrieve transcriptomes from patient donors from SRA
    ...
    # convert retrieved transcriptomes to paired-end fastq files
...
# if the user enters something other than 'test' or 'full', exit the program
    ...
    log_file.close()
    sys.exit('Error: Program Terminated. Please only enter either "test" or "full".')

''' 2 '''
cds_file = open('HCMV_transcriptome.fasta', 'w') #will write all CDS to this file
log_file = open('miniProject.log', 'w') #log file

# use biopython (Entrez efetch) to retrieve genome of HCMV in genbank format from NCBI
...
# make file containing table of sample/condition/path for sleuth
...
# make short R script to find differentially expressed genes between the 2 timepoints using sleuth
sleuth_script = open('sleuth_script.R', 'w')
#noteToSelf: each '' end with \n to create new line, + is visual marker of diff line
...
# run bowtie on each fastq pair, output only reads that align at least once
...
# write the number of reads in each transcriptome before and after bowtie2 mapping to log
log_file = open('miniProject.log', 'a')
...
for i in range(len(before_files)):
    ...
    before_count = 0
    after_count = 0
    for record in SeqIO.parse(before_file, 'fastq'):
        before_count += 1
    for record in SeqIO.parse(after_file, 'fastq'):
        after_count += 1
...
''' 8 '''
# retrieve longest contig from spades assembly and put into a file for use in blast
